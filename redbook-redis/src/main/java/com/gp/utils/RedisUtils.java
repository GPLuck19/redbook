...
public class RedisUtils {
    ...
    public static List<Object> multiGetFromRedisson(String mapKey, List<String> keys) {
        RMap<String, Object> map = CLIENT.getMap(mapKey);
     ...
     * @return 分页数据
     */
    public static <K, V> Map<K, V> getMapByPage(String key, int page, int size) {
        RMap<K, V> rMap = CLIENT.getMap(key);
        List<K> keys = rMap.keySet().stream().sorted().collect(Collectors.toList());
        int start = (page - 1) * size;
        int end = Math.min(keys.size(), page * size);
        if (start >= keys.size()) {
            return Collections.emptyMap();
        }
        List<K> pagedKeys = keys.subList(start, end);
        return rMap.getAll(pagedKeys.stream().collect(Collectors.toSet()));
    }
     ...
     * @return 分页数据
     */
    public static <T> List<T> getListByPage(String key, int page, int size) {
    ...
    public static <T> T getOrLoad(
        ...
        // 如果缓存中不存在，则加载数据库
        T loadedValue = databaseLoader.get();
        if (loadedValue != null) {
            // 存入缓存并设置过期时间
            setCacheObject(key, loadedValue, Duration.ofMillis(timeUnit.toMillis(duration)));
        }
        return loadedValue;
    }
    ...
    public static Collection<String> scanKeys(final String pattern) {
        RKeys rKeys = CLIENT.getKeys();
        Iterable<String> keys = rKeys.getKeysByPattern(pattern, 100); // 每次最多返回 100 个键
