...
import static com.gp.constant.UserConstant.ONLINE_TOKEN_KEY;
...
public class UserActionListener implements SaTokenListener {
    ...
    public void doLogin(String loginType, Object loginId, String tokenValue, SaLoginModel loginModel) {
        ...
        userOnline.setLoginLocation(IpUtils.getRealAddressByIP(ip));
        ...
        if (tokenConfig.getTimeout() == -1) {
            RedisUtils.setCacheObject(ONLINE_TOKEN_KEY + tokenValue, userOnline);
        } else {
            RedisUtils.setCacheObject(ONLINE_TOKEN_KEY + tokenValue, userOnline, Duration.ofSeconds(tokenConfig.getTimeout()));
        }
        // 记录登录日志
        LoginLogEvent loginLogEvent = new LoginLogEvent();
        loginLogEvent.setUsername(username);
        loginLogEvent.setStatus(Constants.LOGIN_SUCCESS);
        loginLogEvent.setMessage(MessageUtils.message("user.login.success"));
    ...
    public void doLogout(String loginType, Object loginId, String tokenValue) {
        RedisUtils.deleteObject(ONLINE_TOKEN_KEY + tokenValue);
